...
def explore(cls:Any, name=None, top=None, depth=0, path=None, access=None, list_type=None, is_list=False, is_optional=False, metadata:Optional[Dict]=None)->None:
    """
    ...
    """
    # wrap[top][cls.__name__].append(f"@pepapi({cls.__name__})\ndef {camel_to_snake(cls.__name__)}(\n    {camel_to_snake(cls.__name__)}:{cls.__name__},")
    wrap[top][cls.__name__].append(f"@pepapi({cls.__name__})\ndef {camel_to_snake(name)}(\n    {camel_to_snake(name)}:{cls.__name__},")
    logging.debug(f"Added API entry ponit:\n@pepapi({cls.__name__})\ndef {camel_to_snake(name)}(\n    {camel_to_snake(name)}:{cls.__name__},")
    
    if depth == 0: # if processing top request or response class
    ...
    for _, parameter in inspect.signature(cls).parameters.items():
        ...
        is_list = False
        is_optional = False
        # populate api entry point "functor" body code with docs and logs
        if not is_scalar_type(cls_):
            ...
            primitives_list = [
                ...
                # f"{('default=' + field.default) if type(field.default) is not MISSING_TYPE else ''}"
                ) 
                for field in fields(cls_) if camel_to_snake(field.name) in primitives_lists[cls_.__name__] 
            ]
            
            if len(primitives_list):
                primitives_doc = f"    the element is populated as:\n    {camel_to_snake(cls_.__name__)}(\n        " + '\n        '.join(primitives_list) + '\n    )'
            
            complex_list = [
                (camel_to_snake(cls_.__name__)+ '.' + camel_to_snake(field.name) + ' # ') + \
                'optional' if field.default is None else 'optional' 
                for field in fields(cls_) if camel_to_snake(field.name) not in primitives_lists[cls_.__name__] 
            ]
            ...
            return_doc = (
                f"    the element population operation returns a reference to the {camel_to_snake(cls_.__name__)} element instance\n"
                "    the element nested element addition operation returns a reference to added element"
            )
            
            wrap[top][cls_.__name__].append('):\n    """')
            if primitives_doc:
                wrap[top][cls_.__name__].append(primitives_doc)
            
            if complex_doc:
                wrap[top][cls_.__name__].append(complex_doc)
            
            wrap[top][cls_.__name__].append(return_doc)
            wrap[top][cls_.__name__].append('    """\n')    
            if primitives_doc:
                params_log = "logger.debug('submitted parameters: " + ','.join([f'{param}=%s' for param in primitives_lists[cls_.__name__]]) + "',\n    " + ', '.join(primitives_lists[cls_.__name__]) + ")"
                wrap[top][cls_.__name__].append(f"\n    {params_log}")
                # element_log = "logger.debug('committed message properties: " + ', '.join([f'{param}=%s' for param in primitives_lists[cls_.__name__]]) + "',\n    " + ', '.join([f'{camel_to_snake(cls_.__name__)}.{param}' for param in primitives_lists[cls_.__name__]]) + ")"
                element_log = "logger.debug('committed message properties: " + ', '.join([f'{param}=%s' for param in primitives_lists[cls_.__name__]]) + "',\n    " + ', '.join([f'{camel_to_snake(name_)}.{param}' for param in primitives_lists[cls_.__name__]]) + ")"
                wrap[top][cls_.__name__].append(f"\n    {element_log}")
            else:
                # wrap[top][cls_.__name__].append("):\n    pass")
                wrap[top][cls_.__name__].append("\n    pass")
...
def parse_args():
    ...
    parser.add_argument("--tmp_api_lib", type=str, default=os.path.join(this_dir, os.path.pardir,  '_dovetail'), help="tmp api lib - will be used to create and populate tmp api modules directory")
    parser.add_argument("--api_lib", type=str, default=os.path.join(this_dir, os.path.pardir,  'dovetail'), help="api lib - will be used to create and populate api modules directory")
    ...
    parser.add_argument("--response_class", type=str, default="Response", help="python class name for schema response sequence")
...
def gen_init_and_json_code(cls):
    ...
    explore(cls, name=name, top=Name, path=list(), access=list())
...
def write_api_entry_points(write_dir_path="../_dovetail", is_write_to_file=False):
    ...
    for key in wrap:
        ...
        print('from decorators import typedlist, pepapi')
        print()
...
if __name__ == "__main__":
    ...
    response_class = getattr(api_lib, args.response_class)
